...
- **Event mapping:** feed slots become `IDiscoveryApiEvent`s with id prefix
  `fsched-`, `type: 'rental'` (renders the existing "Rental" chip), no
  pricing block in the detail modal, and no `linkSEO`, so no register button
  appears (including under `customRegistrationUrl`).
