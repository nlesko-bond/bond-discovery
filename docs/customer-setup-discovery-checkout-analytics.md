...
### Discovery-surface events (fired inside the programs iframe)

The discovery surface intentionally emits a **minimal** event set to your GTM
container — one page view plus registration-intent clicks. The conversion
funnel itself is carried by the Bond checkout events below.

| Event | When | Parameters |
|-------|------|------------|
| `page_view` | Once per programs-page (iframe) load | `page_path`, `page_title`, `page_location`, `page_referrer` |
| `click_register` | Register clicked on programs page | `program_id`, `program_name`, `session_id`?, `session_name`?, `product_id`?, `price`?, `currency` |
| `click_redeem_pass` | Redeem on Schedule tab | `event_id`, `program_id`, `program_name`, `session_id`?, `session_name`? |

`page_view` includes `page_location` (the iframe URL) and `page_referrer` so
your GA4 property can distinguish iframe page views from your own page's
pageviews (see the Step 5 tip about restricting by **Page Hostname**).

### Checkout conversion events (forwarded by the host script)
...
### Migration note — removed discovery events

As of this release the discovery surface no longer emits the following GTM
events: `view_program`, `view_session`, `filter_applied`, `view_mode_changed`,
`schedule_view_changed`, `share_link`, `click_event` (and the internal
`perf_time_to_content` / `perf_events_fetch` pushes). Any GTM triggers or GA4
reports built on those event names stopped receiving data as of this release.
Rebuild funnels on `page_view`, `click_register`, `click_redeem_pass`, and the
forwarded checkout conversion events above.
