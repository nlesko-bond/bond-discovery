...
const MAX_SPACES_UPLOAD_BYTES = 5 * 1024 * 1024;

/** Max filename length surfaced in Slack (bytes via string slice) */
...
export async function POST(
  ...
  if (raw.size > MAX_SPACES_UPLOAD_BYTES) {
    return NextResponse.json({ error: 'File too large (max 5 MB)' }, { status: 400 });
  }

  const fileRecord = raw as File;
  const fileNameRaw = typeof fileRecord.name === 'string' ? fileRecord.name : 'spaces.csv';
  ...
  const storageRelPath = `${org.id}/rentable-spaces-${Date.now()}.csv`;
  ...
  if (uploadError) {
    console.error('upload-spaces-csv: storage error', uploadError);
  ...
  await admin
    .from('orgs')
    .update({
      spaces_upload_storage_path: storageRelPath,
      spaces_upload_original_filename: fileNameRaw.slice(0, 500),
      spaces_uploaded_at: uploadedAtIso,
    })
    .eq('id', org.id);

  await admin.from('activity_log').insert({
    org_id: org.id,
    action: 'spaces_csv_uploaded',
    actor: 'org',
    ...
    if (process.env.SLACK_ONBOARDING_WEBHOOK_URL) {
      ...
      if (notifyCtx) {
        await postOnboardingSlackNotification({
          ...
          footerNote: 'Triggered when rentable-spaces CSV is uploaded from the onboarding checklist.',
          fallbackSuffix: 'Spaces uploaded',
        });
      }
    }
  } catch (e) {
    console.error('upload-spaces-csv: Slack failed', e);
  }
