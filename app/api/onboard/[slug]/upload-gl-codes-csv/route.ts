...
const MAX_GL_CODES_UPLOAD_BYTES = 5 * 1024 * 1024;
...
export async function POST(
  ...
  if (raw.size > MAX_GL_CODES_UPLOAD_BYTES) {
    return NextResponse.json({ error: 'File too large (max 5 MB)' }, { status: 400 });
  }

  const fileRecord = raw as File;
  const fileNameRaw = typeof fileRecord.name === 'string' ? fileRecord.name : 'accounting-codes.csv';
  ...
  const storageRelPath = `${org.id}/accounting-codes-${Date.now()}.csv`;
  ...
  if (uploadError) {
    console.error('upload-gl-codes-csv: storage error', uploadError);
  ...
  await admin
    .from('orgs')
    .update({
      gl_codes_upload_storage_path: storageRelPath,
      gl_codes_upload_original_filename: fileNameRaw.slice(0, 500),
      gl_codes_uploaded_at: uploadedAtIso,
    })
    .eq('id', org.id);

  await admin.from('activity_log').insert({
    org_id: org.id,
    action: 'gl_codes_csv_uploaded',
    actor: 'org',
    ...
    if (process.env.SLACK_ONBOARDING_WEBHOOK_URL) {
      ...
      if (notifyCtx) {
        await postOnboardingSlackNotification({
          headline: 'Accounting codes uploaded',
          ctx: notifyCtx,
          bodyMarkdown:
          ...
          footerNote: 'Triggered when accounting-codes CSV is uploaded from the onboarding checklist.',
          fallbackSuffix: 'GL codes uploaded',
        });
      }
    }
  } catch (e) {
    console.error('upload-gl-codes-csv: Slack failed', e);
  }
