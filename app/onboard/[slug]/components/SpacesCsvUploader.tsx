...
const SPACES_TEMPLATE_HREF = '/onboarding/templates/rentable-spaces.csv';
...
export function SpacesCsvUploader({ slug, uploadedAt, originalFilename }: Props) {
  ...
  const upload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const res = await fetch(`/api/onboard/${encodeURIComponent(slug)}/upload-spaces-csv`, {
          method: 'POST',
          body: fd,
        });
        let body: unknown;
  ...
  return (
    <div className="mt-4 rounded-[8px] border border-dashed border-bond-border bg-bond-bg px-4 py-4">
      <p className="text-[15px] font-medium text-bond-text">Rentable spaces (CSV)</p>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        <a
          href={SPACES_TEMPLATE_HREF}
          download="rentable-spaces-template.csv"
